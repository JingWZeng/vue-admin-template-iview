<template>
    <div class="index-vue">
        ...
        <aside :class="asideClassName">
            <!-- logo -->
            <div class="logo-c">
                <img src="../assets/imgs/egg.png" alt="logo" class="logo" />
                        ...
                        <div v-for="(subItem, i) in item.children" :key="index + i">
                            ...
                            <template v-else-if="!subItem.hidden">
                                <a :href="subItem.url" v-if="subItem.isExternal" target="_blank" class="external">
                                    <MenuItem :class="isShowAsideTitle ? '' : 'shrink'" :name="'external-link-' + index + '-' + i">
                                        <Icon :size="subItem.size" :type="subItem.type" />
                                        <span v-show="isShowAsideTitle">{{ subItem.text }}</span>
                                    </MenuItem>
                                </a>
...
export default {
    ...
    mounted() {
        ...
        this.main = document.querySelector('.sec-right') // 右侧部分
    ...
    methods: {
        ...
        monitorWindowSize() {
            ...
            window.onresize = () => {
                // 可视窗口宽度太小 自动收缩侧边栏
                if (w < 1300 && this.isShowAsideTitle && w > (document.documentElement.clientWidth || document.body.clientWidth)) {
                    this.shrinkAside()
                }
